...
class PublicOfficialAPIContractTest(unittest.TestCase):
    def test_public_frontend_names_are_explicitly_official(self):
        expected = {
        ...
        self.assertTrue(expected.issubset(torch_functional.__all__))
        model = UnifiedSPN(SPNConfig.nlspn(iterations=1))
        self.assertTrue(hasattr(model, "official_frontend"))
        old_frontend_name = "au" + "thor"
        self.assertFalse(hasattr(model, old_frontend_name))
    ...
    def test_each_official_profile_calls_canonical_core_once(self):
