# SPN OfficialFrontend v1 Design
...
boundary to the official module boundary for all five supported profiles:
...
this change. OfficialFrontend v1 is solely a PyTorch reference for the unified SPN
propagation operator.

## Upstream Baseline

The official interfaces and parameter decoders follow these fixed upstream
commits:
...
official modules expose the official plan-producing parameter names and tensor
...
- `spn_accel_cmodel/torch_spn_decoded.py` defines the internal decoded tensor
  contract used only between official modules and plan compilers.
- `spn_accel_cmodel/torch_spn_official_frontend.py` defines five official reference modules
...
- `spn_accel_cmodel/torch_functional.py` exposes the breaking official-frontend
...
`UnifiedSPN(config)` constructs exactly one matching official frontend under
the public `official_frontend` attribute. `forward()` accepts only that
frontend's raw-input type:
...
one of them with the official shape, and copies them into the official module.
OfficialFrontend v1 performs no heuristic key search, prefix guessing, alternate
...
The official interface is `guidance`, `blur_depth`, and optional
`sparse_depth`. CSPN has no learned parameter decoder in the propagation
module. The official module maps:
...
The offset channels are reconstructed in the exact official order with
...
`CompletionFormerOfficialFrontend` shares the NLSPN family implementation and owns the
same official parameter names and shapes. It differs only in profile-specific
configuration:
...
- sparse depth is passed as the official `feat_fix` input.
...
`DySPNOfficialFrontend` owns the released parameter generator:
...
`[B,T,K,2,H,W]` in official YX order; logits are reshaped to `[B,T,K,H,W]`.
...
The official interface names the released tensors directly:

- 48-channel regular-grid guidance;
- `4*T` dynamic logits;
- sparse-depth confidence probability.

The official module reshapes dynamic logits to `[B,T,4,H,W]`; the compiler owns
...
- no tensor is resized, padded to a different official layout, or silently
  substituted;
...
is retained. Official formula NaN/Inf behavior is not clamped or replaced.
...
- residual offsets in official order;
...
The oracle does not call official frontend helpers or plan compilers.

### End-to-end official-boundary equivalence
...
operation-order microcases use exact comparison where the official and canonical
...
- official modules contain parameter decode but no evolving-state recurrence;
...
- the non-official `GENERIC` profile and `compile_generic_plan()` facade are
...
OfficialFrontend v1 is complete only when:

1. all five profiles accept their official-frontend RawInputs;
...
4. all five profiles compile to `CanonicalSPNPlan` and call the single
   recurrence once;
5. every traced propagation iteration matches its independent official formula;
